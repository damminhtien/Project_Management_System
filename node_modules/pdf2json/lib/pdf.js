...
let PDFJSClass = (function () {
    ...
    cls.prototype.getRawTextContent = function(numPages,isPlainText) {
        let retVal = "";
        if (!this.needRawText)
            return retVal;
        var i = 0;
            ...
            _.each(textContent.bidiTexts, function(textObj, idx) {
	            if (prevText) {
		            if (Math.abs(textObj.y - prevText.y) <= 9) {
			            ...
			            if(isPlainText = 0) retVal += prevText.str  + "\r\n";
                        else retVal += prevText.str + " "
			            prevText = textObj;
		            }
	            }
	            else {
